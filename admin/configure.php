...
<?php if (!empty($_POST)):
	...
	if (empty($_POST['factoryReset']) != TRUE ) {
	  ...
	  exec('sudo rm -rf /etc/dstar-radio.mmdvmhost');
	  exec('sudo cp /usr/local/bin/dstar-radio.mmdvmhost /etc/.');
	  ...
	  // Make the root filesystem read-only
          ...
          echo "<h2> PISTAR WILL REBOOT REFRESH YOUR BROWSER IN 50 SECONDS! <h2> ";
	  echo "<br />\n</div>\n";
...
<?php if (file_exists('/etc/dstar-radio.mmdvmhost')) { ?>
    ...
    <td align="left" colspan="2"><select name="mmdvmDisplayType">
	    ...
	    <option <?php if ($configmmdvm['Nextion']['Port'] == "/dev/ttyUSB0") {echo 'selected="selected" ';}; ?>value="/dev/ttyUSB0">/dev/ttyUSB0</option>
 	    <option <?php if ($configmmdvm['Nextion']['Port'] == "/dev/ttyUSB1") {echo 'selected="selected" ';}; ?>value="/dev/ttyUSB1">/dev/ttyUSB1</option>
...
<?php if ( $configmmdvm['POCSAG']['Enable'] == 1 ) { ?>
	...
	<td style="text-align: left;"><select name="pocsagServer">
      ...
      <tr>
        ...
        <td align="left"><input type="text" name="pocsagWhitelist" size="60" maxlength="200" value="<?php if (isset($configdapnetgw['General']['WhiteList'])) { echo $configdapnetgw['General']['WhiteList']; } ?>" /></td>
      </tr>
      <tr>
